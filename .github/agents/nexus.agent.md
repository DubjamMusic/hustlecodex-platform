...
export const config = {
  ...
  // External services
  stripe: {
    publicKey: process.env.NEXT_PUBLIC_STRIPE_KEY ?? (() => { throw new Error('NEXT_PUBLIC_STRIPE_KEY is not set') })(),
    secretKey: process.env.STRIPE_SECRET_KEY ?? (() => { throw new Error('STRIPE_SECRET_KEY is not set') })(),
  },
}
```
